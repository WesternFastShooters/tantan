...
| FE:TC-005 | Auto        | Folo auth mock 有 providers、用户、订阅、read、collection fixture   | 分别完成 Google/GitHub/Apple 官方页→授权令牌、Email、直接授权令牌登录 | 五类入口均建立同一 userId 的 Tantan session，订阅/已读/收藏数与 fixture 相等          |
...
| BE:TC-003 | Auto | auth mock 支持 provider URL、Email 和一次性 token           | 请求非法 provider、重放 token、再完成 Email 和三个社交 provider token | 非法/重放拒绝且无 session；五类入口各只创建一个 Tantan session                                                             |
