...
| FE:TC-004 | Auto | viewport `390x844`，已登录 | 打开 `/` | 显示首页/订阅/设置三项底栏，`DownloadPage` 不在 DOM |
| FE:TC-005 | Auto | Folo auth mock 有用户、订阅、read、collection fixture | 完成 Google→Folo→本地 callback | Home 显示同一 userId，订阅/已读/收藏数与 fixture 相等 |
...
| FE:TC-007 | Auto | 固定四类 HomeCard fixture | 分别以 390/800/1200/1600px 打开 Home | 列数依次为 2/2/3/4，article/post/image/video 均使用对应卡片降级规则 |
...
| FE:TC-016 | Auto | API Key canary `sk-test-DO-NOT-STORE-016` | 测试并保存 Provider，再导出 HAR/日志并扫全部浏览器存储 | canary 仅出现在发往 loopback Go 的当次 request body，response/URL/storage/log/Folo request 均无 |
...
| FE:TC-021 | Manual+Auto | 安装态 PWA，PC/Mobile，reduced-motion 开/关 | 断网、检查 Cache Storage，键盘走完主流程并运行 axe | manifest/install 有效，API/Auth 无缓存，焦点可见，无 critical axe 违规，动效遵守偏好 |
...
| TC | Mode | Setup | Operation | 必须观察到的唯一成功结果 |
|---|---|---|---|---|
| BE:TC-001 | Auto | 空临时数据目录 | 启动服务并请求 health/ready | 迁移 1,2,3 各一次，health=200，ready=200，integrity=ok |
...
| BE:TC-003 | Auto | auth mock 支持一次 token | 发送错 state cookie、重放 token、再完成一次正常 callback | 前两次拒绝且无 session；正常流程只创建一个 local session |
| BE:TC-004 | Auto | Folo token canary 与仿真 Keychain | 登录、请求 session、查 DB/log，再登出 | canary 只存 Keychain，Browser/DB/log 无；登出后 Keychain token 删除 |
...
| BE:TC-012 | Auto | API Key canary 与仿真 Provider | 保存、读配置、调 Provider，扫 response/DB/log | canary 仅存 Keychain 和发往 Provider 的 Authorization，配置 GET 只返回 hasApiKey/fingerprint |
| BE:TC-013 | Auto | Provider 依次 timeout/429/500，同时提交3任务 | 运行 worker，并尝试用户提供 URL/内网 IP | 超时/重试次数符合合同，并发最大2，任意 URL 输入校验失败且不 dial |
...
| BE:TC-023 | Auto | 依次注入端口占用、DB锁、Keychain失败、DNS/TLS失败 | 执行 `tantan-api doctor` | 每个故障都有稳定检查名/结果/恢复建议，输出不含 Token/Key/Prompt/正文 |

## 4. 总门禁

```bash
bash spec-package/scripts/validate-package.sh
python3 /Users/mingrui/.codex/skills/frontend-spec/scripts/validate_spec.py 2026-08-09-tantan-frontend-spec.md --domain frontend --stage final
python3 /Users/mingrui/.codex/skills/backend-spec/scripts/validate_spec.py 2026-08-09-tantan-backend-spec.md --domain backend --stage final
pnpm typecheck
pnpm lint
...
本期尚未导入 Folo/编写代码时，只执行前三个规格包门禁；业务实现开始后，所有适用命令都是完成必要条件。
