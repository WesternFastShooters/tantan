...
| J-07       | 未登录                   | 任意需登录路由 | 显示与 Folo 相同的 Google、GitHub、Apple、Email 和授权令牌入口；Email 由同源 Go 登录，若 Folo 要求 TOTP 则在原页输入验证码；社交按钮打开 Folo 官方登录页，完成后把单次授权令牌粘贴回 Tantan 由 Go 兑换；成功进入原目标 | 错误留在登录页；provider/2FA 流可取消；刷新可恢复 session | `/login?returnTo=`；Folo 官方页在新 tab | 建立不透明 Tantan 会话且所有 Folo 登录方式可用 |
...
| ------ | ------ | -------- | ----------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------ | ------------------------ | ------------------------- | --------------------------------- |
| API-01 | CMP-11 | Go       | GET `/api/auth/folo/providers`；POST `/api/auth/folo/social-start`、`/email`、`/two-factor`、`/token` | provider、email/password、flowId/TOTP 或一次性 token、returnTo | providers、固定 Folo authorize URL、2FA challenge 或 session user | provider 非法 400；登录/令牌无效 401；2FA 409/过期 410 | anonymous + exact Origin | 15s，不自动重试           | 成功清 user cache；敏感字段不缓存 |
